...
namespace FieldRegistry
{
    ...
    static const std::unordered_map<std::string, JsonEntry> s_json_registry = {
        ...
        { "Map::Markers::Quests",
          { "Array of active quest-marker destinations (the markers Skyrim renders as the floating quest arrows on the compass and as quest-target icons on the world map). On SE/AE this uses PlayerCharacter::questTargets plus TESQuest::IsActive()/QuestFlag::kActive; Miscellaneous targets honor the journal's master Miscellaneous toggle (Journal_QuestsTab::unk30). Coordinates are resolved native-first: TESQuestTarget::GetTrackingRef (the engine's own tracked reference), then the engine's TeleportPath door chain (start/end when that end lies in a top-level worldspace), then BGSLocation::worldLocMarker ancestors, then the interior cell's exit door. localX/localY/localZ keep the raw target reference coordinates; targets with no resolvable global position are omitted rather than published with local coordinates. Targets resolving to the same destination are collapsed. Each entry: { questFormId, questEditorId, questName, questType, isActive, isMiscellaneous, objectiveIndex, objectiveText, objectiveTextResolved, aliasId, refId, isDeleted, name, coordinateSource, coordinateRefId, coordinateRefName, localX, localY, localZ, localWorldspace, localWorldspaceFormId, localParentWorldspace, localParentWorldspaceFormId, localCell, localCellFormId, localIsInterior, x, y, z, worldspace, worldspaceFormId, parentWorldspace, parentWorldspaceFormId, cell, cellFormId, isInterior }. objectiveText keeps the raw template (may contain <Alias=...> placeholders for radiant quests); objectiveTextResolved resolves those aliases through the current quest instance data where possible.",
            "array",
            &QuestMarkers::ReadQuestMarkers } },
        { "Debug::Map::Markers::Quests",
          { "Debug snapshot for quest marker diagnosis: runtime module, Miscellaneous master-toggle state (Journal_QuestsTab::unk30), every tracked quest target with its native TeleportPath dump and resolved map coordinates, and the current Map::Markers::Quests output.",
            "object",
