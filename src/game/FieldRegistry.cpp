...
namespace FieldRegistry
{
    ...
    static const std::unordered_map<std::string, JsonEntry> s_json_registry = {
        ...
        { "Map::Markers::Quests",
          { "Array of active quest-objective targets (the markers Skyrim renders as the floating quest arrows on the compass and as quest-target icons on the world map). Only quests flagged active in the journal are returned. Each entry: { questFormId, questEditorId, questName, questType, isActive, objectiveIndex, objectiveText, objectiveTextResolved, aliasId, refId, name, x, y, z, worldspace, worldspaceFormId, parentWorldspace, parentWorldspaceFormId, cell, cellFormId, isInterior }. objectiveText keeps the raw template (may contain <Alias=...> placeholders for radiant quests); objectiveTextResolved has those tokens replaced with alias display names where possible.",
            "array",
