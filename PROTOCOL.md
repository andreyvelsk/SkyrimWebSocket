...
```jsonc
{
  ...
  "command": "equip",       // command name (required): equip | unequip | use | drop | favorite | equip_spell | unequip_spell | favorite_spell | set_active_quest | hotkey_set | hotkey_clear | hotkey_trigger
  "formId": "0x00012EB7",  // item / spell / quest form ID as hex string (required for most commands)
...
| `command` | **yes** | — | One of: `equip`, `unequip`, `use`, `drop`, `favorite`, `equip_spell`, `unequip_spell`, `favorite_spell`, `set_active_quest`, `hotkey_set`, `hotkey_clear`, `hotkey_trigger`. |
| `formId` | conditional | — | Hex form ID of the target item / spell / quest. Required for all commands **except** `hotkey_clear` and `hotkey_trigger`. |
