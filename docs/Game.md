...
| `inDialogue`       | `boolean` | `true` while the player is engaged in a dialogue / conversation with an NPC. Combines `RE::PlayerCharacter::IsInDialogue()` and the `Dialogue Menu` being open. |
